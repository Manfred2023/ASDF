...
class Reply
{
    ...
    #[NoReturn] static public function _error_create($result, int $code = 409): void
    {
        http_response_code($code);
        echo json_encode([self::STATUS => 0,
            ((is_array($result) && !empty($result)) ? self::RESPONSE : self::MESSAGE) => $result]);
    ...
    #[NoReturn] static public function _error_field($result, int $code = 400): void
    {
        http_response_code($code);
        echo json_encode([self::STATUS => 0,
            ((is_array($result) && !empty($result)) ? self::RESPONSE : self::MESSAGE) => $result]);
    ...
    #[NoReturn] static public function _error_found($result, int $code = 404): void
    {
        http_response_code($code);
        echo json_encode([self::STATUS => 0,
            ((is_array($result) && !empty($result)) ? self::RESPONSE : self::MESSAGE) => $result]);
